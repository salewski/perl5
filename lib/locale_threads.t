...
# specify the categories.  This way both individual setting and LC_ALL gets
# tested.
my $lc_all_frequency = 3;
...
sub add_trials($$;$)
{
  ...
  LOCALE:
    foreach my $locale (@locales) {
        ...
        # different alternate.
        for (my $i = 1; $i < $deterministic_trial_count; $i++) {
        ...
        # except to get more edit distance
        # need.  Note this assumes that the same op isn't used in two
        # categories.
        if (0 && $op && defined $op_counts{$op} && $op_counts{$op} >= $thread_count)
        {
...
SKIP: {
    ...
    if (${^O} eq "MSWin32" && $Config{'libc'} !~ /ucrt/) {
        for (my $i = 0; $i < @locales; $i++) {
            ...
            setlocale($LC_ALL, "Albanian");    # Defeat any attempt to skip the
                                        # setlocale if the same as current
    ...
    foreach my $category (keys %distincts) {
        ...
        for my $op_result (sort keys $distincts{$category}->%*) {
            ...
            print STDERR __FILE__, ": ", __LINE__, ": ", Dumper $distincts{$category}{$op_result} if $debug > 1;
        ...
        print STDERR __FILE__, ": ", __LINE__, ": count", Dumper \%distinct_ops if $debug > 1;
        if ($debug > 1) {
            print STDERR __FILE__, ": ", __LINE__, ":\n", Dumper \%results;
            Dump %results, 1000000;
            ...
            foreach my $result ($results_ref->{$op}->@*) {
                ...
                if ($debug > 1) {
                    print STDERR __FILE__, ": ", __LINE__, ": $op--$result\n",
                                Dumper $distincts_ref->{$op_result}{locales};
                    Dump $op_result, 1000000;
                    print STDERR "End of Dump\n";
                }
                push @return, {
        ...
        if ($debug > 1) {
            print STDERR __FILE__, ": ", __LINE__, ": ", Dumper \@ops, \%results, $distincts{$category};
            Dump \@ops, 1000000;
            print STDERR "End of Dump\n";
            Dump \%results, 1000000;
            print STDERR "End of Dump\n";
            Dump $distincts{$category}, 1000000;
            print STDERR "End of Dump\n";
        }
    ...
    if ($debug) {
        print STDERR __FILE__, ": ", __LINE__, ": ", Dumper $all_tests_ref;
        #Dump $all_tests_ref->@*, 1000000;
        #print STDERR __FILE__, ": ", __LINE__, ": End of dump\n";
    }
    ...
    # Finally ready to run the test.  This stresses locale thread safety.  We
    # start a bunch of threads.  Each sets the locale of each category being
    # tested to the value determined in the code above.  Then each sleeps
    # to a common start time, at which point they awaken and iterate their
    ...
    # threads interfering.  The code above has set up things so that each
    # thread has as disparate results from the other threads as possible, so
    # to more likely catch any bleed-through.
    fresh_perl_is("
    BEGIN { \$| = 1; }
        use threads;
        use strict;
            ...
            if (! \$utf8_matches) {
                if (utf8::is_utf8(\$copy)) {
                    ...
                    print STDERR \" (result wasn't in UTF-8; converted for\",
                                 \" easier comparison)\";
                    use Devel::Peek;
                    #print STDERR __FILE__, \": \", __LINE__, \": \"; Dump \$result;
                }
            }
            print STDERR \":\\n\";
